...
CREATE TABLE IF NOT EXISTS `BestRide`.`User` (
  ...
  `UserInfo_idUserInfo` INT NOT NULL,
  PRIMARY KEY (`idUser`, `UserInfo_idUserInfo`),
  UNIQUE INDEX `password_UNIQUE` (`password` ASC),
  INDEX `fk_User_UserInfo1_idx` (`UserInfo_idUserInfo` ASC),
  CONSTRAINT `fk_User_UserInfo1`
    FOREIGN KEY (`UserInfo_idUserInfo`)
    REFERENCES `bestride`.`user_info` (`idUserInfo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
